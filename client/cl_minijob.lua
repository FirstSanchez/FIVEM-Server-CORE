local currentJobKey = nil
	...
	function StartCount(key)
	    ...
	    TriggerEvent("cataleya_hud:sendNotify", "info", "JOB-SYSTEM", "Fahre zum nächsten Wegpunkt", 5000)	
	...
	function Interact(key)
		...
		if currentJobKey ~= nil and currentJobKey ~= key then
            TriggerEvent("cataleya_hud:sendNotify", "error", "JOB-SYSTEM", "Du bist bereits in einem anderen Job. Bitte beende ihn vorher!", 5000)
			return
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'menu', {
			...
			if data.current.value == 'stop' then
				ESX.TriggerServerCallback('diaz_jobs:getkey', function(has)
					if has then
                        ...
                        TriggerEvent("cataleya_hud:sendNotify", "info", "JOB-SYSTEM", "Du hast den gekündigt.", 5000)
						currentJobKey = nil
						currentVehicle = nil
						checkpointCount = 0
						SetWaypointOff()
			...
			elseif data.current.value == 'starten' then
							...
							if flag == false then
                                TriggerEvent("cataleya_hud:sendNotify", "info", "JOB-SYSTEM", "Spawnpunkt belegt. Bitte warte kurz.", 5000)
								return
							end
