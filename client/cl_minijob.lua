...
function StartCount(key)
	...
	ESX.ShowNotification("Fahre zum nächsten Wegpunkt")
...
function Interact(key)
	...
	if currentJobKey ~= nil and currentJobKey ~= key then
		ESX.ShowNotification("Du bist bereits in einem anderen Job. Bitte beende ihn vorher!")
		return
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'menu', {
		...
		if data.current.value == 'stop' then
			ESX.TriggerServerCallback('diaz_jobs:getkey', function(has)
				if has then
					...
					ESX.ShowNotification("Du hast den Job gekündigt.")
					currentJobKey = nil
					currentVehicle = nil
					checkpointCount = 0
					SetWaypointOff()
		...
		elseif data.current.value == 'starten' then
						...
						if flag == false then
							ESX.ShowNotification("Spawnpunkt belegt. Bitte warte kurz.")
							return
						end
