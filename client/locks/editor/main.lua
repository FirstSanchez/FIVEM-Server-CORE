...
function refresh_controls(sub_state, extra_bool) 
    ...
    if state ==  E_EDITOR_STATES.OVERVIEW then
        active_controls = {
            { text = "Beenden", codes = {CfgLock.Keys.BACK}},
            { text = "Neues Schloss", codes = {CfgLock.Keys.INTERACTION}},
        }
    elseif state ==  E_EDITOR_STATES.CREATE_SELECTION then
        table.insert(active_controls, { text = "Zurück", codes = {CfgLock.Keys.BACK}})
        
        if sub_state ~= 3 and extra_bool then
            table.insert(active_controls, { text = "Weiter", codes = {CfgLock.Keys.INTERACTION}})
        end

        if sub_state == 1 or sub_state == 2 then
            table.insert(active_controls, {
                text = (sub_state == 1 and "Tür hinzufügen" or "Tür entfernen"), codes = {CfgLock.Keys.SELECT_DOORS}
            })
        elseif sub_state == 3 then
            table.insert(active_controls, {
                text = "Einzel", codes = {CfgLock.Keys.INTERACTION2}
            })
            table.insert(active_controls, {
                text = "Mehrfach", codes = {CfgLock.Keys.INTERACTION3}
            })
        end
    elseif state ==  E_EDITOR_STATES.INFO then
        table.insert(active_controls, { text = (sub_state == 2 and "Abbrechen" or "Beenden"), codes = {CfgLock.Keys.BACK}})

        if sub_state ~= 2 then
            table.insert(active_controls, {
                text = "Löschen", codes = {CfgLock.Keys.INTERACTION4}
            })
            
            if sub_state == 1 then
                table.insert(active_controls, {
                    text = "Einfügen", codes = {CfgLock.Keys.INTERACTION3}
                })
            end
          
            table.insert(active_controls, {
                text = "Kopieren", codes = {CfgLock.Keys.INTERACTION2}
            })

            table.insert(active_controls, {
                text = "Bearbeiten", codes = {CfgLock.Keys.INTERACTION}
            })
        else
            table.insert(active_controls, {
                text = "Bestätigen", codes = {CfgLock.Keys.INTERACTION}
            })
        end
    end
...
function state_overview()
    ...
    -- keybinds
    if IsDisabledControlJustReleased(0, CfgLock.Keys.INTERACTION) then
        state = E_EDITOR_STATES.CREATE_SELECTION
        selected_doors = {} -- reset selection
    elseif IsDisabledControlJustReleased(0, CfgLock.Keys.BACK) then
        editor.stop()
    end
end
...
function state_create_select()
    ...
    if IsDisabledControlJustReleased(0, CfgLock.Keys.SELECT_DOORS) then
    ...
    elseif IsDisabledControlJustReleased(0, CfgLock.Keys.INTERACTION) and extra_bool == true then
        sub_state = 3
    elseif IsDisabledControlJustReleased(0, CfgLock.Keys.INTERACTION2) and sub_state == 3 then
    ...
    elseif IsDisabledControlJustReleased(0, CfgLock.Keys.INTERACTION3) and sub_state == 3 then
        state = E_EDITOR_STATES.CREATE_SETTINGS
...
function state_info()
   ...
   -- change state if not aiming on a door anymore
    ...
    if IsDisabledControlJustReleased(0, CfgLock.Keys.BACK) then
        if sub_state == 2 then
            sub_state = 0 
        else
            editor.stop()
    ...
    if IsDisabledControlJustReleased(0, CfgLock.Keys.INTERACTION4) then
        sub_state = 2
    end

    if lock ~= nil and IsDisabledControlJustReleased(0,  CfgLock.Keys.INTERACTION2) then
        sub_state = 1
        copied_lock = Helpers.Table.Clone(lock)
    end

    if IsDisabledControlJustReleased(0,  CfgLock.Keys.INTERACTION3) and sub_state == 1 and copied_lock ~= nil and lock ~= nil then
    ...
    if lock ~= nil and IsDisabledControlJustReleased(0, CfgLock.Keys.INTERACTION) then 
        if sub_state == 0 then
...
function main_loop()
    ...
    DisableControlAction(0, CfgLock.Keys.SELECT_DOORS, true)
    DisableControlAction(0, CfgLock.Keys.INTERACTION, true)
    DisableControlAction(0, CfgLock.Keys.INTERACTION2, true)
    DisableControlAction(0, CfgLock.Keys.INTERACTION3, true)
    DisableControlAction(0, CfgLock.Keys.INTERACTION4, true)
    DisableControlAction(0, CfgLock.Keys.BACK, true)
