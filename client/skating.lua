...
RegisterNetEvent("sanchez-scripts:client:start", function() Skating.Start() end)
...
Skating.Start = function()
	...
	while DoesEntityExist(Skating.Entities) and DoesEntityExist(Skating.Player) do Wait(5)
		...
		if distance <= cf_skating.LoseConnectionDistance then
...
Skating.HandleKeys = function(distance)
	...
	if distance < cf_skating.LoseConnectionDistance then
		local overSpeed = (GetEntitySpeed(Skating.Entities)*3.6) > cf_skating.MaxSpeedKmh
		...
		if IsControlPressed(0, 22) and Connected then
			if not IsEntityInAir(Skating.Entities) then	
				...
				boosting = cf_skating.maxJumpHeigh * duration / 250.0
				if boosting > cf_skating.maxJumpHeigh then boosting = cf_skating.maxJumpHeigh end
...
Skating.Spawn = function()
	...
	Skating.Board = CreateObject(GetHashKey("p_defilied_ragdoll_01_s"), spawnCoords, true, true, true)
...
Skating.Connect = function(param)
	...
	if param == "placeskateboard" then
		...
		ESX.ShowNotification(cf_skating.Language.Info['controls']) -- Comment if you don't wanna use this
...
Skating.ConnectPlayer = function(toggle)
	if toggle then
		...
		TriggerServerEvent("sanchez-scripts:server:skate")
...
RegisterNetEvent("sanchez-scripts:client:skate", function(id)
	local player = GetPlayerFromServerId(id)
	local vehicle = GetEntityAttachedTo(GetPlayerPed(player))
end)
-- Howdy #4 INSERT skateboard