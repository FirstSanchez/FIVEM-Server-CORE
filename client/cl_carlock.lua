...
CreateThread(function()
    ...
    RegisterCommand("carlock", function ()
        if not CARLOCK_COOLDOWN then
            ...
            notowned = 0
            if #cars == 0 then
                TriggerEvent("prime_notify","info", "Fahrzeug", "Fahrzeug nicht gefunden.", 5000)
            else
                for j=1, #cars, 1 do
                    ...
                    if(ownedVehiclePlates[plate]) then
                        if hasAlreadyLocked ~= true then
                            ...
                            if lock == 1 or lock == 0 then
                                ...
                                TriggerEvent("prime_notify","info", "Fahrzeug", "Du hast dein Fahrzeug abgeschlossen.", 5000)
                            ...
                            elseif lock == 2 then
                                ...
                                TriggerEvent("prime_notify", "info","Fahrzeug", "Dein Fahrzeug wurde aufgeschlossen.", 5000)
                                if not IsPedInAnyVehicle(PlayerPedId(), true) then
                                    TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
                                end
                                SetVehicleLights(carstrie[i], 2)
                        ...
                        ESX.TriggerServerCallback('lm_carlock:isVehicleOwner', function(owner)
                            if owner and hasAlreadyLocked ~= true then
                                ...
                                elseif lock == 2 then
                                    SetVehicleDoorsLocked(carstrie[i], 1)
                                    PlayVehicleDoorOpenSound(carstrie[i], 0)
                                    TriggerEvent("prime_notify", "info","Fahrzeug", "Du hast dein Fahrzeug aufgeschlossen.", 5000)
                                    SendNUIMessage({
                                        ui = "carlock",
                                        lock = false
                                    })
                            ...
                            if notowned == #carstrie then
                                TriggerEvent("prime_notify", "info","LIMITLESS | Fahrzeug", "Fahrzeug nicht gefunden.", 5000)
                            end	
                        end, plate)
                    end
                end			
