...
function callTakeHostage()
	...
	if not canTakeHostage then 
		TriggerEvent('prime_notify', 'error', 'Information', 'Information", "Du benotigst eine Pistole mit Munition, um eine Geisel mit vorgehaltener Waffe zu nehmen!', 5000)
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerEvent('prime_notify', 'error', 'Information', 'Information", "Niemand in der Umgebung, der als Geisel genommen werden kann!', 5000)
			end
		else
			TriggerEvent('prime_notify', 'error', 'Information', 'Information", "Niemand in der Umgebung, der als Geisel genommen werden kann!', 5000)
		end
...
CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Halte [G] um los zu lassen, [H] zum killen")
