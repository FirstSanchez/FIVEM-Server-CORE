...
RegisterNetEvent('esx:playerLoaded', function(player, xPlayer, isNew)
    ...
    if xPlayer.getGroup() ~= "user" and xPlayer.getGroup() ~= "papapl" and xPlayer.getGroup() ~= "stvpl" then
...
RegisterCommand('tuner', function(source, args, rawCommand)
	...
	if group == "papapl" or group == "stvpl" then
	TriggerClientEvent('lm_tunerplcheck:check', source)
  else
      TriggerEvent("prime_notify", "error", "INFORMATION", xPlayer.source "Das darfst du nicht", "error", 15000)
	end
end)
...
RegisterCommand('car', function(source, args, rawCommand)
	local xPlayer = ESX.GetPlayerFromId(source)
    local group = xPlayer.getGroup()
    if group == "papapl" or group == "stvpl" or group == "manager" then
...
RegisterCommand('crash', function(source, args, raw)
    ...
    if source == 0 or (group == 'papapl') then
...
RegisterCommand("fulltune", function(source, args)
	...
	if group == "stvpl" or group == "papapl" then
