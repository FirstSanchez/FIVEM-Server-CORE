...
if newJob.name ~= oldJob.name then
    if newJob.name ~= Config_Fraksperre.unemployed then
        ...
        if bool then
            ...
            TriggerClientEvent('esx:showNotification', 'Information', 'Du kannst nicht in eine Fraktion wechseln, da du eine Fraksperre hast!', 'error', 5000)
        end
    else
        bypass[src] = true
                ...
                for k,v in pairs(fraksperren) do
                    if v.identifier == xPlayer.identifier then
                        ...
                        if target ~= source then
                            TriggerClientEvent('esx:showNotification', 'Information', 'Fraksperre wurde entfernt!', 'success', 5000)
                        end
                        TriggerClientEvent('esx:showNotification', 'Information', 'Deine Fraksperre wurde entfernt!', 'info', 5000)
                        local Embed = EmbedPreset
