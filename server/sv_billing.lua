		...
		TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
				...
				function(rowsChanged)
					xTarget.showNotification('Sie haben eine Rechnung erhalten.')
				end)
			else
					...
					if xPlayer.getMoney() >= amount then
						...
						function(rowsChanged)
							if rowsChanged == 1 then
								...
								xPlayer.showNotification(('Sie bezahlen eine Rechnung von $%s.'):format(ESX.Math.GroupDigits(amount)))
								xTarget.showNotification(('Sie erhalten eine Zahlung von $%s.'):format(ESX.Math.GroupDigits(amount)))
							end
