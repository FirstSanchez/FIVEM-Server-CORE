...
CreateThread(function()
	...
	for k, v in pairs(Config_Tattoo.ShopsTattoos) do
		local blip = AddBlipForCoord(v)
		SetBlipSprite(blip, 75)
		SetBlipColour(blip, 3)
		SetBlipScale(blip, 0.8)
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName("DMTattoos")
...
function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Öffne Tattoo Laden", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Ändere Kameransicht", 21, "Ändere Stärke", {90, 89}, "Kaufe/Entferne ein Tattoo", 191)
		scaleString = sType
	end
end
...
CreateThread(function()
	...
	for k, v in ipairs(Config_Tattoo.TattooCats) do
    ...
    while true do 
		...
		local CanSleep = true
		if not IsMenuOpen() then
			for _,interiorId in ipairs(Config_Tattoo.interiorIds) do
		...
		for k, v in ipairs(Config_Tattoo.TattooCats) do
			if JayMenu.IsMenuOpened(v[1]) then
				...
				for _, tattoo in pairs(Config_Tattoo.AllTattooList) do
