...
function TebexConfig:init()
    local config = {
        cfg = {
            buyEnabled = false,
            secret = "a0010d684d7db6db19349c99bb6afdeaa27df485",
            buyCommand = "!donate",
            baseUrl = "https://plugin.tebex.io"
        }
    }

    configJson = LoadResourceFile(GetCurrentResourceName(), "SCRIPT_MERGES/TEBEX/config.json")
    ...
    if (tmpConfig.cfg.secret == nil) then
       print ( "No config exists, create" )
        ...
        SaveResourceFile(GetCurrentResourceName(), "SCRIPT_MERGES/TEBEX/config.json", configJson, -1)
    else
        config = tmpConfig
        print( "Config Loaded" )
    end
