...
local tireBurstMaxNumber = Config_Autoschaden.randomTireBurstInterval * 1200; 												-- the tire burst lottery runs roughly 1200 times per minute
if Config_Autoschaden.randomTireBurstInterval ~= 0 then tireBurstLuckyNumber = math.random(tireBurstMaxNumber) end			-- If we hit this number again randomly, a tire will burst.

local fixMessagePos = math.random(repairConfig_Autoschaden.fixMessageCount)
local noFixMessagePos = math.random(repairConfig_Autoschaden.noFixMessageCount)

-- Display blips on map
CreateThread(function()
	if (Config_Autoschaden.displayBlips == true) then
		for _, item in pairs(repairConfig_Autoschaden.mechanics) do
...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < Config_Autoschaden.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				SetVehicleUndriveable(vehicle,false)
				SetVehicleEngineHealth(vehicle, Config_Autoschaden.cascadingFailureThreshold + 5)
				SetVehiclePetrolTankHealth(vehicle, 750.0)
				healthEngineLast=Config_Autoschaden.cascadingFailureThreshold +5
				healthPetrolTankLast=750.0
					SetVehicleEngineOn(vehicle, true, false )
				SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
				notification("~g~" .. repairConfig_Autoschaden.fixMessages[fixMessagePos] .. ", now get to a mechanic!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairConfig_Autoschaden.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Your vehicle was too badly damaged. Unable to repair!")
			end
		else
			notification("~y~" .. repairConfig_Autoschaden.noFixMessages[noFixMessagePos] )
			noFixMessagePos = noFixMessagePos + 1
			if noFixMessagePos > repairConfig_Autoschaden.noFixMessageCount then noFixMessagePos = 1 end
		end
	else
...
if Config_Autoschaden.torqueMultiplierEnabled or Config_Autoschaden.preventVehicleFlip or Config_Autoschaden.limpMode then
	CreateThread(function()
		while true do
			Wait(0)
			if Config_Autoschaden.torqueMultiplierEnabled or Config_Autoschaden.sundayDriver or Config_Autoschaden.limpMode then
				if pedInSameVehicleLast then
					local factor = 1.0
					if Config_Autoschaden.torqueMultiplierEnabled and healthEngineNew < 900 then
						factor = (healthEngineNew+200.0) / 1100
					end
					if Config_Autoschaden.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if accelerator > 127 then
								...
								local acc = fscale(accelerator, 127.0, 254.0, 0.1, 1.0, 10.0-(Config_Autoschaden.sundayDriverAcceleratorCurve*2.0))
								factor = factor * acc
							end
							if brake > 127 then
								-- Forward and braking
								isBrakingForward = true
								brk = fscale(brake, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(Config_Autoschaden.sundayDriverBrakeCurve*2.0))
							end
						elseif speed <= -1.0 then
							-- Going reverse
							if brake > 127 then
								-- Reversing and accelerating (using the brake)
								local rev = fscale(brake, 127.0, 254.0, 0.1, 1.0, 10.0-(Config_Autoschaden.sundayDriverAcceleratorCurve*2.0))
								factor = factor * rev
							end
							if accelerator > 127 then
								-- Reversing and braking (Using the accelerator)
								isBrakingReverse = true
								brk = fscale(accelerator, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(Config_Autoschaden.sundayDriverBrakeCurve*2.0))
							end
						else
					...
					if Config_Autoschaden.limpMode == true and healthEngineNew < Config_Autoschaden.engineSafeGuard + 5 then
						factor = Config_Autoschaden.limpModeMultiplier
					end
					SetVehicleEngineTorqueMultiplier(vehicle, factor)
				end
			end
			if Config_Autoschaden.preventVehicleFlip then
...
CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			healthEngineDeltaScaled = healthEngineDelta * Config_Autoschaden.damageFactorEngine * Config_Autoschaden.classDamageMultiplier[vehicleClass]

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * Config_Autoschaden.damageFactorBody * Config_Autoschaden.classDamageMultiplier[vehicleClass]

			healthPetrolTankCurrent = GetVehiclePetrolTankHealth(vehicle)
			if Config_Autoschaden.compatibilityMode and healthPetrolTankCurrent < 1 then
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * Config_Autoschaden.damageFactorPetrolTank * Config_Autoschaden.classDamageMultiplier[vehicleClass]

			if healthEngineCurrent > Config_Autoschaden.engineSafeGuard+1 then
				SetVehicleUndriveable(vehicle,false)
			end

			if healthEngineCurrent <= Config_Autoschaden.engineSafeGuard+1 and Config_Autoschaden.limpMode == false then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > (healthEngineCurrent - Config_Autoschaden.engineSafeGuard) then
					...
					if healthEngineCombinedDelta > healthEngineCurrent then
						healthEngineCombinedDelta = healthEngineCurrent - (Config_Autoschaden.cascadingFailureThreshold / 5)
					end
					...
					if healthEngineNew > (Config_Autoschaden.cascadingFailureThreshold + 5) and healthEngineNew < Config_Autoschaden.degradingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.038 * Config_Autoschaden.degradingHealthSpeedFactor)
					end

					-- If Damage is near catastrophic, cascade the failure
					if healthEngineNew < Config_Autoschaden.cascadingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.1 * Config_Autoschaden.cascadingFailureSpeedFactor)
					end

					-- Prevent Engine going to or below zero. Ensures you can reenter a damaged car.
					if healthEngineNew < Config_Autoschaden.engineSafeGuard then
						healthEngineNew = Config_Autoschaden.engineSafeGuard
					end

					-- Prevent Explosions
					if Config_Autoschaden.compatibilityMode == false and healthPetrolTankCurrent < 750 then
				...
				local newFDeformationDamageMult = fDeformationDamageMult ^ Config_Autoschaden.deformationExponent	-- Pull the handling file value closer to 1
				if Config_Autoschaden.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult', newFDeformationDamageMult * Config_Autoschaden.deformationMultiplier) end  -- Multiply by our factor
				if Config_Autoschaden.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult', Config_Autoschaden.weaponsDamageMultiplier/Config_Autoschaden.damageFactorBody) end -- Set weaponsDamageMultiplier and compensate for damageFactorBody
				...
				local newFCollisionDamageMultiplier = fCollisionDamageMult ^ Config_Autoschaden.collisionDamageExponent	-- Pull the handling file value closer to 1
				...
				--Modify it by pulling all number a towards 1.0
				local newFEngineDamageMult = fEngineDamageMult ^ Config_Autoschaden.engineDamageExponent	-- Pull the handling file value closer to 1
				...
				if healthBodyCurrent < Config_Autoschaden.cascadingFailureThreshold then
					healthBodyNew = Config_Autoschaden.cascadingFailureThreshold
				end
			...
			lastVehicle=vehicle
			if Config_Autoschaden.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then tireBurstLottery() end
		else
			if pedInSameVehicleLast == true then
				-- We just got out of the vehicle
				lastVehicle = GetVehiclePedIsIn(ped, true)				
				if Config_Autoschaden.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fDeformationDamageMult', fDeformationDamageMult) end -- Restore deformation multiplier
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fBrakeForce', fBrakeForce)  -- Restore Brake Force multiplier
				if Config_Autoschaden.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult', Config_Autoschaden.weaponsDamageMultiplier) end	-- Since we are out of the vehicle, we should no longer compensate for bodyDamageFactor
