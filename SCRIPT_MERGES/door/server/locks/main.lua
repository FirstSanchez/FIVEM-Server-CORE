CreateThread(function()
    ...
    RegisterNetEvent('ax_locks:sync:locks')
    AddEventHandler('ax_locks:sync:locks', function()
        TriggerClientEvent("ax_locks:update-locks", source, Locks)
    end)
    ...
    MySQL.ready(function() 
        MySQL.query("SELECT * FROM ax_locks", {}, function(result)
            Locks = {}
    ...
    function HasPermissionDoorlocks(lock, playerId)
        local player = ESX.GetPlayerFromId(playerId)

        if CfgLocks.isMaster(player.identifier, player.group) then
            return true
        end

        for i, group in ipairs(CfgLocks.GroupWhitelist) do
            if group == player.group then return true end
        end

        for i, license in ipairs(CfgLocks.IdentifierWhitelist) do
            if license == player.identifier then return true end
        end

        for i, job in ipairs(CfgLocks.Jobwhitelist) do
            if job == player.job.name then return true end
            if job == player.job2.name then return true end
        end

        if CfgLocks.SecondJob then
            for i, job in ipairs(lock.permissions.jobs) do
                if job.name == player.job2.name and job.min_grade <= player.job2.grade then return true end
            end
        end
        for i, job in ipairs(lock.permissions.jobs) do
    ...
    AddEventHandler('ax_locks:toggle', function(lockId, inVehicle)
        ...
        for i, lock in ipairs(Locks) do
            if lock.id == lockId then 
                if HasPermissionDoorlocks(lock, _source) then
    ...
    AddEventHandler('ax_locks:delete', function(lockId)
        ...
        MySQL.update("DELETE FROM ax_locks WHERE id=@id", {['@id'] = lockId},
    ...
    AddEventHandler('ax_locks:create-locks', function(locks)
        ...
        for i, lock in ipairs(locks) do
            MySQL.insert("INSERT INTO ax_locks SET options=@options",{['@options'] = json.encode(lock)},
            function(id)
                for j,v in ipairs(lock.doors) do
                    lock.doors[j].id = tostring(id) .. tostring(j)
                end

                MySQL.update("UPDATE ax_locks SET options=@options WHERE id=@id", {['@id'] = id, ['@options'] = json.encode(lock)},
