...
ESX = nil
  ...
  function OpenPlasticSurgeryNormal()
      HasPaid = false
      needstopay = Config_schoenheitsklinik.Zones[CurrentZone].price 
  ...
  CreateThread(function()
      while true do
          ...
          for k,v in pairs(Config_schoenheitsklinik.Zones) do
              if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config_schoenheitsklinik.DrawDistance) then
                sleep = 0
                  canSleep = false
  ...
  function refreshBlips()
      if Config_schoenheitsklinik.EnableBlips then
          if Config_schoenheitsklinik.EnableUnemployedOnly then
              if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'unemployed' or ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'gang' then
                  for k,v in pairs(Config_schoenheitsklinik.Locations) do
                      local blip = AddBlipForCoord(v.x, v.y)
  
                      SetBlipSprite (blip, Config_schoenheitsklinik.BlipPlasticSurgery.Sprite)
                      SetBlipDisplay(blip, Config_schoenheitsklinik.BlipPlasticSurgery.Display)
                      SetBlipScale  (blip, Config_schoenheitsklinik.BlipPlasticSurgery.Scale)
                      SetBlipColour (blip, Config_schoenheitsklinik.BlipPlasticSurgery.Color)
  ...
  CreateThread(function()
      ...
      if Config_schoenheitsklinik.EnablePeds then
          for _, item in pairs(Config_schoenheitsklinik.Locations) do
